...
import { UserCircleIcon, MailIcon, LockClosedIcon } from './icons';
...
const SignupPage: React.FC<SignupPageProps> = ({ onSignup, onSwitchToLogin }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const newUser = await db.addUser({ name, email, password, role: 'User' });
            onSignup(newUser);
        } catch (err) {
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
            <div className="w-full max-w-md space-y-8">
                <div className="text-center">
                    ...
                    <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">Create your account.</p>
                </div>
                         ...
                         <div>
                            <label htmlFor="name" className="sr-only">Full Name</label>
                             <div className="relative">
                                <span className={iconClasses}><UserCircleIcon /></span>
                                <input
                                    id="name"
                                    name="name"
                                    type="text"
